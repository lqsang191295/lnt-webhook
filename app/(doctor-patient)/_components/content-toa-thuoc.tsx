...
import { formatStringDateToDDMMYYYY } from "@/utils/timer";
...
export default function ContentToaThuoc() {
  ...
  const toaThuocData =
    data.data?.length > 0 ? (data.data as iToaThuoc[]) : null;
                ...
                <AccordionTrigger className="px-4 py-2 text-left">
                  <div className="flex flex-col">
                    ...
                    <span className="text-sm text-muted-foreground">
                      Ngày kê: {formatStringDateToDDMMYYYY(toa.Ngay)} • Bác sĩ:{" "}
                      {toa.TenBsKeToa} • Khoa: {toa.TenKhoa}
                    </span>
                    {toa.Ghichu && (
                        ...
                        <div
                          ...
                          <div className="flex justify-between items-center mb-1">
                            ...
                            <Badge variant="secondary">
                              {" "}
                              Hàm lượng: {thuoc.Hamluong} - Hoạt chất:{" "}
                              {thuoc.Hoatchat} - Số lượng: {thuoc.Soluong} - Đơn
                              vị: {thuoc.Donvi}
                            </Badge>
                          </div>
                          <div className="text-sm text-muted-foreground">
                            Sáng: {thuoc.SLSang || 0} - Trưa:{" "}
                            {thuoc.SLTrua || 0} - Chiều: {thuoc.SLChieu || 0} -
                            Tối: {thuoc.SLToi || 0}
                          </div>
                          {thuoc.Ghichu && (
                            <div className="text-sm text-orange-600 mt-1">
                              Ghi chú: {thuoc.Ghichu}
                            </div>
                          )}
                        </div>
                      ))}
